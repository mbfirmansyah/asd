...
curl -X POST -d '{"nw_src": "'$src'", "nw_dst": "'$dst'", "nw_proto": "ICMP", "actions": "DENY", "priority": "10"}' http://localhost:8080/firewall/rules/0000000000000001
rule1s1=`curl -s http://localhost:8080/firewall/rules/0000000000000001 | jq ' .[] | .access_control_list | .[] | .rules | max_by(.rule_id) | .rule_id'`

echo

curl -X POST -d '{"nw_src": "'$dst'", "nw_dst": "'$src'", "nw_proto": "ICMP", "actions": "DENY", "priority": "10"}' http://localhost:8080/firewall/rules/0000000000000001
